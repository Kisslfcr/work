...
class UsersSpider(scrapy.Spider):
    ...
    def parse(self, response):
        item = UserItem(
            ...
            status = response.css('div.user-status span::text').extract_first(default='wu').strip(),
            school_job = response.xpath('//div[@class="user-status"]/span[2]/text()').extract_first(default='wu').strip(),
